...
public:
    vector<string> generateParenthesis(int n) {
        vector<string> res;// result output string 
        string op = "";
        int open_brct = n;// number of total opening Parentheses
        int close_brct = n;// number of total closing Parentheses
    ...
    void solve(int open_brct , int close_brct,string op,vector<string>& res){
        ...
        // we can include as many number of opening Parentheses as we want  until we are left with opening Parentheses 
        if(open_brct != 0){
            string op1 = op;
            op1.push_back('(');
       ...
       // after inserting close Parentheses we will decrease the total close Parentheses by 1 
            solve(open_brct,close_brct-1,op1,res);
        }
        return ;
    }
};
